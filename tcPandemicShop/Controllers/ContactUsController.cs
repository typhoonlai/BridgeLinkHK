        ...
        */
        ...
        public bool SendEmailToAdmin(string contactName, string contactEmail, string contactMessage)
        {
            try
            {
                ...
                //add sender email
                //from and to the same email: info@goldenswallow.biz
                //confirm the sender email info@goldenswallow.biz  - you cannot use anyone else  (ie gmail or hotmail account) as the from address.

                MailMessage mailMessage = new MailMessage();

                mailMessage.From = new System.Net.Mail.MailAddress(ConfigurationManager.AppSettings["emailAdminFrom"]);
                mailMessage.To.Add(new System.Net.Mail.MailAddress(ConfigurationManager.AppSettings["emailAdminTo"]));
                //testing  mailMessage.To.Add(new System.Net.Mail.MailAddress("amanullaha@chrc.net"));
                mailMessage.Subject = "Potential Customer contacting Golden Swallow";

                mailMessage.IsBodyHtml = true;
                //AlternateView htmlView = AlternateView.CreateAlternateViewFromString(GetRegistrationEmailBody(string.Empty, string.Empty, string.Empty, string.Empty), null, "text/html");
                AlternateView htmlView = AlternateView.CreateAlternateViewFromString("Contact Name: " + contactName + "<br/>" + "Contact Email: " + contactEmail + "<br/>" + "Message: " + contactMessage + "<br/>" + "From Location: goldenswallow.biz", null, "text/html");
        ...
        public void SendGridMail(MailMessage Message)
        {
            ...
            try
            {
                ...
                //create Create a sender identity
                //use Domain Authentication
                //to create 4 CName entries in dns server
